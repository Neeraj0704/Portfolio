import nodemailer from "nodemailer";
import { queryResume, chatWithGemini } from "./llm.js"; // Gemini + RAG logic
import fs from "fs";
import path from "path";

// Register routes
export function registerRoutes(app) {
    // 📩 Contact form route
    app.post("/api/contact/send", async (req, res) => {
        const { name, email, subject, message } = req.body;
        console.log("📥 Incoming contact form:", req.body);

        if (!name || !email || !message) {
            console.warn("⚠️ Missing required fields");
            return res.status(400).json({ error: "Please fill in all required fields." });
        }

        try {
            const transporter = nodemailer.createTransport({
                service: "gmail",
                auth: {
                    user: "neerajvpattanashetti@gmail.com",
                    pass: "ssfz lamn hnoi ezon", // Gmail App Password
                },
            });

            await transporter.sendMail({
                from: `"${name}" <${email}>`,
                to: "neerajvpattanashetti@gmail.com",
                subject: subject || "New Contact Form Message",
                text: `From: ${name} <${email}>\n\n${message}`,
            });

            console.log("✅ Contact form email sent successfully");
            res.json({ success: true, message: "Message sent successfully" });
        } catch (error) {
            console.error("❌ Error sending email:", error);
            res.status(500).json({ error: "Failed to send message" });
        }
    });

    // 🤖 Chat route (Gemini + Google TTS)
    app.post("/api/chat", async (req, res) => {
        try {
            const { query } = req.body;
            console.log("🗣️ User query:", query);

            if (!query) {
                return res.status(400).json({ error: "Query is required" });
            }

            // 1️⃣ Query vector DB for context
            const contextDocs = await queryResume(query);
            console.log("📚 Retrieved context docs:", contextDocs?.length || 0);

            // 2️⃣ Call Gemini + Google TTS
            const { text, audioBase64 } = await chatWithGemini(query, contextDocs);
            console.log("🤖 Gemini response:", text);

            // 3️⃣ Check if audio was generated
            if (audioBase64) {
                console.log("🎶 Audio generated successfully");
                // Save audio file locally for debugging
                const audioDir = path.join(process.cwd(), "audio");
                if (!fs.existsSync(audioDir)) {
                    fs.mkdirSync(audioDir, { recursive: true });
                }
                const audioPath = path.join(audioDir, "response.mp3");
                fs.writeFileSync(audioPath, Buffer.from(audioBase64, "base64"));
                console.log(`💾 Audio saved at: ${audioPath}`);
            } else {
                console.warn("⚠️ No audio generated by Google TTS");
            }

            // 4️⃣ Send back text + audio (base64)
            res.json({
                text,
                audioBase64,
            });
        } catch (err) {
            console.error("❌ Chat error:", err);
            res.status(500).json({ error: "Internal Server Error" });
        }
    });
}
